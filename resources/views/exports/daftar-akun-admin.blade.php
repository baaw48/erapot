...
<head>
    ...
    <title>Daftar Akun Admin - {{ $sekolah->nama_sekolah ?? 'Sekolah' }}</title>
    <style>
...
<div class="page">
    ...
    <div class="warning-banner">
        ...
        <span>RAHASIA — Password telah direset. Dokumen ini berisi password baru yang masih dapat dibaca. Harap simpan dengan aman dan serahkan langsung kepada admin yang bersangkutan. (Note: Password akun Anda sendiri tidak direset)</span>
    </div>
    @else
    <div class="warning-banner info">
        <span class="warning-icon">ℹ️</span>
        <span>Password tidak ditampilkan karena tidak direset. Gunakan opsi "Reset Password" untuk mencetak password baru.</span>
    </div>
    @endif
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge" style="background:#f0fdf4; color:#166534; border:1px solid #bbf7d0;">Admin</span></td>
                    <td><span class="badge badge-username">{{ $akun['username'] }}</span></td>
                    <td>
                        @if($resetPassword && strpos($akun['password'], 'tidak direset') === false)
                            <span class="badge badge-password">{{ $akun['password'] }}</span>
                        @else
                            <span class="badge badge-no-reset">{{ $akun['password'] }}</span>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" style="text-align:center; color:#94a3b8; padding:32px;">Belum ada data admin.</td>
                </tr>
                @endforelse
            </tbody>
        </table>

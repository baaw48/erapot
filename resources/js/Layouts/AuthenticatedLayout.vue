        ...
        >
            ...
            <nav class="flex-grow px-3 py-2 overflow-y-auto scrollbar-hide space-y-0.5">
                ...
                <SidebarLink v-if="userRole === 'admin'" :href="route('kehadiran.index')" :active="route().current('kehadiran.index')" :isCollapsed="isSidebarCollapsed">
                    ...
                    Kehadiran & Ekskul
                    <template #sublinks v-if="!isSidebarCollapsed">
                        <div class="space-y-1.5 px-3">
                            <Link :href="route('sekolah.index')"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300"
                                :class="route().current('sekolah.*') 
                                    ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold' 
                                    : 'text-slate-600 dark:text-slate-400 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/80 hover:text-slate-900 dark:hover:text-slate-200'">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                                    :class="route().current('sekolah.*') ? 'bg-blue-100 dark:bg-blue-900/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'">
                            ...
                            <Link :href="route('tahun-ajaran.index')"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300"
                                :class="route().current('tahun-ajaran.*') 
                                    ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold' 
                                    : 'text-slate-600 dark:text-slate-400 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/80 hover:text-slate-900 dark:hover:text-slate-200'">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                                    :class="route().current('tahun-ajaran.*') ? 'bg-blue-100 dark:bg-blue-900/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                </div>
                                Tahun Ajaran
                            </Link>

                            <Link :href="route('ekskul.index')"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300"
                                :class="route().current('ekskul.*') 
                                    ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold' 
                                    : 'text-slate-600 dark:text-slate-400 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/80 hover:text-slate-900 dark:hover:text-slate-200'">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                                    :class="route().current('ekskul.*') ? 'bg-blue-100 dark:bg-blue-900/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'">

    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-black text-xl text-slate-800 leading-tight tracking-tight">Kenaikan & Kelulusan</h2>
        ...
        <div class="animate-fade-in space-y-6 max-w-7xl mx-auto pb-12">
            ...
            <div class="flex gap-4 p-2 bg-slate-100 rounded-2xl max-w-md mx-auto relative z-10">
                <button @click="tipeMode = 'kenaikan'" :class="tipeMode === 'kenaikan' ? 'bg-white shadow-sm text-brand-600' : 'text-slate-500 hover:text-slate-700'" class="flex-1 py-2.5 text-sm font-bold rounded-xl transition-all">
                    Kenaikan Kelas
                </button>
            ...
            <div class="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.02)] relative animate-slide-up" style="animation-delay: 0.05s; animation-fill-mode: both;">
                ...
                <div v-if="kelasAsalId" class="relative z-10">
                    <div class="bg-slate-50 border-y border-slate-100 px-8 py-4 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <input type="checkbox" id="selectAll" v-model="selectAll" @change="toggleSelectAll" class="w-5 h-5 rounded-md border-slate-300 text-brand-600 focus:ring-brand-500 cursor-pointer">
                        ...
                        <div class="text-xs font-black text-slate-500 uppercase tracking-widest">
                            Terpilih: <span class="text-brand-600 text-sm">{{ selectedSiswas.length }}</span> / {{ siswas.length }}
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="(siswa, index) in siswas" :key="siswa.id" class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="px-8 py-4 w-12">
                                        <input type="checkbox" :id="'siswa_'+siswa.id" :value="siswa.id" v-model="selectedSiswas" class="w-5 h-5 rounded-md border-slate-300 text-brand-600 focus:ring-brand-500 cursor-pointer">
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <label :for="'siswa_'+siswa.id" class="flex items-center gap-4 cursor-pointer">
                                            ...
                                            <div>
                                                <div class="text-sm font-bold text-slate-800">{{ siswa.nama_siswa }}</div>
        ...
        <Modal :show="showConfirmModal" @close="showConfirmModal = false" maxWidth="sm">
            <div class="p-8 text-center relative overflow-hidden">
                <div class="absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl opacity-20 pointer-events-none" :class="tipeMode === 'kenaikan' ? 'bg-brand-500' : 'bg-indigo-500'"></div>
                <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full mb-6 relative z-10" :class="tipeMode === 'kenaikan' ? 'bg-brand-50 text-brand-500' : 'bg-indigo-50 text-indigo-500'">
                ...
                <h2 class="text-xl font-black text-slate-800 mb-2 relative z-10">Konfirmasi Proses</h2>
                <p class="text-sm font-semibold text-slate-500 mb-8 relative z-10">
                    Apakah Anda yakin ingin <strong :class="tipeMode === 'kenaikan' ? 'text-brand-600' : 'text-indigo-600'">{{ tipeMode === 'kenaikan' ? 'Menaikkan' : 'Meluluskan' }}</strong> {{ selectedSiswas.length }} siswa yang dipilih?
                </p>
                <div class="flex gap-3 relative z-10">
                    <button @click="showConfirmModal = false" class="flex-1 px-4 py-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-bold rounded-xl transition-all">
                        Batal
                    </button>

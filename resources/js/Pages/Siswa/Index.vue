    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-black text-xl text-slate-800 leading-tight tracking-tight">Data Siswa</h2>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Kelola Peserta Didik Aktif</p>
                </div>
            </div>
        </template>
        ...
        <div class="animate-fade-in space-y-6 max-w-7xl mx-auto pb-12">
            ...
            <div v-if="$page.props.flash && $page.props.flash.success" class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 px-5 py-4 rounded-2xl shadow-sm mb-4" role="alert">
            ...
            <div v-if="$page.props.flash && $page.props.flash.error" class="flex items-center gap-3 bg-rose-50 border border-rose-200 text-rose-700 px-5 py-4 rounded-2xl shadow-sm mb-4" role="alert">
            ...
            <div class="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.02)] relative animate-slide-up" style="animation-delay: 0.05s; animation-fill-mode: both;">
                ...
                <div class="p-6 border-b border-slate-100 bg-white/50 backdrop-blur-sm relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex-1 w-full flex flex-col md:flex-row gap-4">
                        <div class="flex-1 md:max-w-xs">
                            <label for="search" class="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Cari Siswa</label>
                            <div class="relative">
                                <input id="search" v-model="searchQuery" type="text" placeholder="Nama, NIS, atau NISN..." class="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-4 py-2.5 pr-10 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all" />
                                <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                    <button v-if="searchQuery" @click="searchQuery = ''" class="p-1 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Hapus Pencarian">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    </button>
                                    <div v-else class="p-1 text-slate-400 pointer-events-none">
                        ...
                        <div class="flex-1 md:max-w-xs">
                            <label for="filter" class="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Filter Kelas</label>
                            <div class="relative">
                                <select id="filter" v-model="filterKelasId" class="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-4 py-2.5 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all appearance-none cursor-pointer">
                        ...
                        <div class="w-full md:w-32 shrink-0">
                            <label for="perPage" class="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tampilkan</label>
                            <div class="relative">
                                <select id="perPage" v-model="perPage" class="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-4 py-2.5 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all appearance-none cursor-pointer">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="hidden md:block text-right mr-2">
                            ...
                            <p class="text-xl font-black text-brand-600">{{ siswas.total }}</p>
                        </div>
                        <div class="flex gap-2 w-full md:w-auto">
                            ...
                            <a :href="filterKelasId ? route('siswa.export', { kelas_id: filterKelasId }) : route('siswa.export')" target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300 text-sm font-bold rounded-xl shadow-sm transition-all">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                                ...
                                <th class="px-8 py-4">Nomor Induk</th>
                                <th class="px-8 py-4">Nama Siswa</th>
                                <th class="px-8 py-4 text-center">L/P</th>
                                <th class="px-8 py-4">Kelas</th>
                                <th class="px-8 py-4 text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            <tr v-for="(siswa, index) in siswas.data" :key="siswa.id" class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-6 py-5 text-center text-sm font-bold text-slate-400">
                                    {{ (siswas.current_page - 1) * siswas.per_page + index + 1 }}
                                </td>
                                <td class="px-8 py-5">
                                    <div class="text-sm font-bold text-slate-700">{{ siswa.nis }}</div>
                                    <div class="text-[11px] font-bold text-slate-400 mt-0.5">{{ siswa.nisn || 'Tanpa NISN' }}</div>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-3">
                                        <div class="h-8 w-8 rounded-full flex items-center justify-center font-black text-xs shrink-0"
                                ...
                                <td class="px-8 py-5 text-center">
                                    <span v-if="siswa.jenis_kelamin === 'L'" class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-blue-50 text-blue-600 font-bold text-xs border border-blue-100" title="Laki-laki">L</span>
                                    <span v-else class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-pink-50 text-pink-600 font-bold text-xs border border-pink-100" title="Perempuan">P</span>
                                </td>
                                <td class="px-8 py-5">
                                ...
                                <td class="px-8 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button @click="openEditModal(siswa)" class="p-2.5 text-brand-500 hover:bg-brand-50 rounded-xl transition-colors" title="Edit Siswa">
                                        ...
                                        <button @click="confirmDelete(siswa.id)" class="p-2.5 text-rose-500 hover:bg-rose-50 rounded-xl transition-colors" title="Hapus Siswa">
                            ...
                            <tr v-if="siswas.data.length === 0">
                                <td colspan="6" class="px-8 py-16 text-center">
                                    ...
                                    <p class="text-xs font-medium text-slate-400 mt-1">Belum ada data siswa untuk kelas ini.</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <!-- Paginasi -->
                <div class="px-8 py-4 border-t border-slate-100 bg-slate-50" v-if="siswas.links && siswas.links.length > 3">
        ...
        <Modal :show="showModal" @close="closeModal" maxWidth="md">
            <div class="p-8 relative overflow-hidden">
                ...
                <div class="flex items-center gap-4 mb-8 relative z-10">
                    ...
                    <div>
                        <h2 class="text-xl font-black text-slate-800">{{ isEditing ? 'Edit Data Siswa' : 'Tambah Siswa Baru' }}</h2>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Lengkapi data pribadi siswa</p>
                    </div>
                </div>
                
                <form @submit.prevent="submit" class="space-y-5 relative z-10">
                    <div class="space-y-2">
                        <label for="nama_siswa" class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">Nama Lengkap Siswa</label>
                        <input id="nama_siswa" v-model="form.nama_siswa" type="text" class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all placeholder:font-normal placeholder:text-slate-400" required placeholder="Cth: Ahmad Fulan" />
                        <div v-if="form.errors.nama_siswa" class="text-rose-500 text-xs font-bold mt-1">{{ form.errors.nama_siswa }}</div>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label for="nis" class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">NIS</label>
                            <input id="nis" v-model="form.nis" type="text" class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all placeholder:font-normal placeholder:text-slate-400" required />
                            <div v-if="form.errors.nis" class="text-rose-500 text-xs font-bold mt-1">{{ form.errors.nis }}</div>
                        </div>
                        <div class="space-y-2">
                            <label for="nisn" class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">NISN <span class="text-[10px] text-slate-400 font-medium">(Opsional)</span></label>
                            <input id="nisn" v-model="form.nisn" type="text" class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all placeholder:font-normal placeholder:text-slate-400" />
                            <div v-if="form.errors.nisn" class="text-rose-500 text-xs font-bold mt-1">{{ form.errors.nisn }}</div>
                        </div>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label for="jenis_kelamin" class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">Jenis Kelamin</label>
                            <select id="jenis_kelamin" v-model="form.jenis_kelamin" class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all appearance-none cursor-pointer" required>
                                <option value="L">Laki-laki (L)</option>
                                <option value="P">Perempuan (P)</option>
                            </select>
                            <div v-if="form.errors.jenis_kelamin" class="text-rose-500 text-xs font-bold mt-1">{{ form.errors.jenis_kelamin }}</div>
                        </div>
                        <div class="space-y-2">
                            <label for="kelas_id" class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">Kelas / Rombel</label>
                            <select id="kelas_id" v-model="form.kelas_id" class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 font-bold focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all appearance-none cursor-pointer" required>
                                <option value="" disabled class="text-slate-400">-- Pilih Kelas --</option>
                                <option v-for="k in kelas" :key="k.id" :value="k.id">{{ k.nama_kelas }}</option>
                            </select>
                            <div v-if="form.errors.kelas_id" class="text-rose-500 text-xs font-bold mt-1">{{ form.errors.kelas_id }}</div>
                        </div>
                    </div>
        ...
        <Modal :show="showDeleteModal" @close="closeDeleteModal" maxWidth="sm">
            <div class="p-8 text-center relative overflow-hidden">
                ...
                <p class="text-sm font-semibold text-slate-500 mb-8 relative z-10">
                    Apakah Anda yakin ingin menghapus data siswa ini? <strong class="text-rose-600">Seluruh nilai, absensi, dan data rapor siswa akan ikut terhapus!</strong>
                </p>
        ...
        <Modal :show="showImportModal" @close="closeImportModal" maxWidth="md">
            <div class="p-8 relative overflow-hidden">
                ...
                <div class="flex items-center gap-4 mb-6 relative z-10">
                    ...
                    <div>
                        <h2 class="text-xl font-black text-slate-800">Import Data Siswa</h2>
                ...
                <div class="relative z-10">
                    <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                        ...
                        <ul class="text-xs text-amber-700 space-y-1 list-disc pl-4 font-medium">
                            ...
                            <li>Pastikan "Kelas" yang ditulis di Excel <strong>sama persis</strong> dengan kelas yang terdaftar.</li>
                            <li>Jika NIS sudah ada, data lama akan di-update otomatis.</li>
                        </ul>
                        <a href="/download_siswa.php" target="_blank" class="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-brand-600 hover:text-brand-700">

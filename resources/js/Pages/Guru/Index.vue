    ...
    <AuthenticatedLayout>
                ...
                <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a :href="route('guru.export')" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 bg-white dark:bg-slate-700 border border-slate-600 dark:border-slate-500 text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:bg-slate-600 text-sm font-medium rounded-xl transition-all">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            <tr v-for="(guru, index) in gurus.data" :key="guru.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-semibold text-slate-700 dark:text-slate-200 dark:text-slate-200">{{ guru.name }}</div>
                                            <div class="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{{ guru.nip ? 'NIP. ' + guru.nip : '-' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 text-xs font-medium">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button @click="openEditModal(guru)" class="p-2 text-slate-400 dark:text-slate-500 hover:text-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg transition-colors">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </button>
                                        <button @click="confirmDelete(guru.id)" class="p-2 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors">
                            ...
                            <tr v-if="gurus.data.length === 0">
                                <td colspan="5" class="px-6 py-16 text-center">
                                    ...
                                    <p class="font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">Belum ada data guru</p>
                                    <p class="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Tambahkan data guru baru</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="p-6">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Lengkapi profil dan penugasan</p>
                    </div>
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">NIP</label>
                    ...
                    <div>
                        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Username</label>
                    ...
                    <div v-if="isEditing || !isEditing" class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Password <span v-if="isEditing" class="text-slate-400 dark:text-slate-500 font-normal">(opsional)</span></label>
                        ...
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Konfirmasi</label>
        ...
        <Modal :show="showDeleteModal" @close="showDeleteModal = false" maxWidth="sm">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-6">Yakin hapus guru ini? Data tidak dapat dikembalikan.</p>

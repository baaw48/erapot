    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-0.5">Kelola Rombongan Belajar</p>
                </div>
            </div>
        </template>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <button @click="openAddModal" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition-all">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            <tr v-for="(k, index) in kelas.data" :key="k.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 dark:text-slate-300 text-xs font-semibold">{{ k.tingkat }}</span>
                                </td>
                                <td class="px-6 py-4">
                            ...
                            <tr v-if="kelas.data.length === 0">
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <p class="font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">Belum ada kelas</p>
                                    <p class="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Tambahkan kelas baru</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" v-if="kelas.links?.length > 3">
        ...
        <Modal :show="showModal" @close="closeModal" maxWidth="md">
            <div class="p-6">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Nama Kelas</label>
                        <input v-model="form.nama_kelas" type="text" required placeholder="Contoh: VII-A, VIII-1" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 rounded-xl px-4 py-3 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all" />
                        <div v-if="form.errors.nama_kelas" class="text-xs text-danger-500 mt-1">{{ form.errors.nama_kelas }}</div>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Tingkat</label>
                            <select v-model="form.tingkat" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 rounded-xl px-4 py-3 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        ...
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">KKTP</label>
                            <input v-model="form.kktp" type="number" min="0" max="100" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 rounded-xl px-4 py-3 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all" />
                            <div v-if="form.errors.kktp" class="text-xs text-danger-500 mt-1">{{ form.errors.kktp }}</div>
                        </div>
                    </div>

                    <div>
                        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5 block">Wali Kelas</label>
        ...
        <Modal :show="showDeleteModal" @close="showDeleteModal = false" maxWidth="sm">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-6">Yakin hapus kelas ini? Data siswa & nilai ikut terhapus.</p>
        ...
        <Modal :show="showSuccessModal" @close="showSuccessModal = false" maxWidth="sm">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-5">Data kelas berhasil disimpan.</p>

    ...
    <AuthenticatedLayout>
        ...
        <div class="px-4 pb-6 space-y-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h3 class="font-bold text-slate-800 dark:text-white text-lg">Daftar Kelas</h3>
                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ kelas.total }} kelas terdaftar</p>
                </div>
                <button @click="openAddModal" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition-all">
            ...
            <div class="hidden md:block bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                ...
                                <th class="px-4 py-3.5 text-center">KKTP</th>
                            ...
                            <tr v-for="(k, index) in kelas.data" :key="k.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-xl bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold shadow-inner text-lg">
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold">{{ k.tingkat }}</span>
                                </td>
                                <td class="px-4 py-4">
                                    <div v-if="k.wali_kelas" class="flex items-center gap-2">
                                        <div class="h-7 w-7 rounded-full bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400 flex items-center justify-center text-xs font-bold uppercase border border-white dark:border-slate-600 shadow-sm">
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span class="text-sm font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1 rounded-lg border border-blue-100 dark:border-blue-800">{{ k.kktp }}</span>
                                </td>
                ...
                <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" v-if="kelas.links?.length > 3">
            ...
            <div class="md:hidden space-y-3">
                <div v-for="(k, index) in kelas.data" :key="k.id" class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200/50 dark:border-slate-700/50 p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="h-12 w-12 rounded-xl bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-xl shadow-inner">
                                {{ k.nama_kelas.charAt(0) }}
                            </div>
                            <div>
                                <h4 class="font-bold text-slate-800 dark:text-white text-lg">{{ k.nama_kelas }}</h4>
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">Kelas {{ k.tingkat }}</span>
                                    <span class="text-xs font-bold px-2 py-0.5 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800">KKTP: {{ k.kktp }}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700/50">
                        <div class="flex items-center gap-2">
                            <svg class="h-4 w-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            <span class="text-sm text-slate-500 dark:text-slate-400">Wali Kelas:</span>
                            <span v-if="k.wali_kelas" class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ k.wali_kelas.name }}</span>
                            <span v-else class="text-sm text-slate-400 dark:text-slate-500">Belum ada</span>
                        </div>
                    </div>
                </div>

                <!-- Empty State Mobile -->
                <div v-if="kelas.data.length === 0" class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200/50 dark:border-slate-700/50 p-8 text-center">
                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-700 text-slate-300 dark:text-slate-500 mb-4">
                        <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    </div>
                    <p class="font-semibold text-slate-500 dark:text-slate-400">Belum ada kelas</p>
                    <button @click="openAddModal" class="mt-4 px-4 py-2 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition-all">
                        Tambah Kelas Baru
                    </button>
                </div>

                <!-- Pagination Mobile -->
                <div v-if="kelas.links?.length > 3" class="flex justify-center">
                    <Pagination :links="kelas.links" />
                </div>
            </div>
        </div>
            ...
            <div class="p-5 dark:bg-slate-800">
                <div class="flex items-center gap-3 mb-5">
                    <div class="p-2.5 bg-primary-100 dark:bg-primary-900/50 text-primary-600 dark:text-primary-400 rounded-xl">
                        ...
                        <svg v-else class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                    </div>
                    <div>
                        <h2 class="text-base font-bold text-slate-800 dark:text-white">{{ isEditing ? 'Edit Kelas' : 'Tambah Kelas Baru' }}</h2>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        ...
                        <input v-model="form.nama_kelas" type="text" required placeholder="Contoh: VII-A" class="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 rounded-xl px-3.5 py-2.5 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all" />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select v-model="form.tingkat" class="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 rounded-xl px-3.5 py-2.5 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        ...
                        <div>
                            ...
                            <input v-model="form.kktp" type="number" min="0" max="100" class="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 rounded-xl px-3.5 py-2.5 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all" />
                    ...
                    <div>
                        ...
                        <SearchableSelect v-model="form.wali_kelas_id" :options="guruOptions" placeholder="-- Kosong --" searchPlaceholder="Ketik nama guru..." />
                    ...
                    <div class="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-700 mt-5">
                        <button type="button" @click="closeModal" class="flex-1 px-4 py-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-slate-600 transition-all text-sm">Batal</button>
                        <button type="submit" :disabled="form.processing" class="flex-1 px-4 py-2.5 bg-gradient-to-r from-primary-600 to-primary-500 text-white font-semibold rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60 flex items-center justify-center gap-2 text-sm">
        ...
        <Modal :show="showDeleteModal" @close="showDeleteModal = false" maxWidth="sm">
            <div class="p-5 text-center dark:bg-slate-800">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-5">Yakin hapus kelas ini? Data siswa & nilai ikut terhapus.</p>
                <div class="flex gap-3">
                    <button @click="showDeleteModal = false" class="flex-1 px-4 py-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-slate-600 transition-all text-sm">Batal</button>
                    <button @click="executeDelete" class="flex-1 px-4 py-2.5 bg-danger-500 text-white font-medium rounded-xl hover:bg-danger-600 transition-all text-sm">Hapus</button>
                </div>
            </div>
        </Modal>
        ...
        <Modal :show="showSuccessModal" @close="showSuccessModal = false" maxWidth="sm">
            <div class="p-5 text-center dark:bg-slate-800">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Data kelas berhasil disimpan.</p>
                <button @click="showSuccessModal = false" class="px-6 py-2.5 bg-primary-600 text-white font-medium rounded-xl hover:bg-primary-700 transition-all text-sm">Tutup</button>
            </div>
        </Modal>
    </AuthenticatedLayout>
</template>